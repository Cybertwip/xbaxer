...
var (
	importRE      = regexp.MustCompile(`\Aimport\s+(\.|[A-Za-z0-9_]+)?\s*"([^"]+)"\s*(//.*)?\r?\n?\z`)
	importBlockRE = regexp.MustCompile(`\A\s*(?:(\.|[A-Za-z0-9_]+)?\s*"([^"]+)")?\s*(//.*)?\r?\n?\z`)
)
...
func bootstrapFixImports(srcFile string) string {
	...
	for i, line := range lines {
		lineNoEOL := strings.TrimSuffix(strings.TrimSuffix(line, "\n"), "\r")
		if strings.HasSuffix(lineNoEOL, "*/") {
			inComment = false
		}
		if strings.HasSuffix(lineNoEOL, "/*") {
			inComment = true
		...
		if strings.HasPrefix(lineNoEOL, "import (") {
			inBlock = true
			continue
		}
		if inBlock && strings.HasPrefix(lineNoEOL, ")") {
			inBlock = false
			continue
		}
