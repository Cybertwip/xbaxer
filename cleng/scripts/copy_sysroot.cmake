...
set(_sysroot_marker_found "")
if(DEFINED SYSROOT_MARKERS AND NOT SYSROOT_MARKERS STREQUAL "")
  foreach(_marker IN LISTS SYSROOT_MARKERS)
    if(EXISTS "${SYSROOT_SOURCE}/${_marker}")
      set(_sysroot_marker_found "${_marker}")
      break()
    endif()
  endforeach()
else()
  if(EXISTS "${SYSROOT_SOURCE}/${SYSROOT_MARKER}")
    set(_sysroot_marker_found "${SYSROOT_MARKER}")
  endif()
endif()

if(_sysroot_marker_found STREQUAL "")
  if(DEFINED SYSROOT_MARKERS AND NOT SYSROOT_MARKERS STREQUAL "")
    message(STATUS "cleng target bundle: none of [${SYSROOT_MARKERS}] found under ${SYSROOT_SOURCE}; skipping bundle")
  else()
...
if(DEFINED SYSROOT_SKIP_IF_PRESENT AND SYSROOT_SKIP_IF_PRESENT AND EXISTS "${SYSROOT_DEST}/${_sysroot_marker_found}")
...
function(cleng_copy_resolved_tree source_root rel_path dest_root resolved_ancestors)
  ...
  if(IS_DIRECTORY "${_resolved_source}")
    # Apple SDK frameworks contain directory symlinks that point back into
    # an ancestor directory (for example Ruby.framework/Headers/ruby/ruby -> .).
    # Flattening those links recursively would create unbounded
    # ruby/ruby/ruby/... paths that later fail on Windows/Xbox installs.
    list(FIND resolved_ancestors "${_resolved_source}" _ancestor_index)
    if(NOT _ancestor_index EQUAL -1)
      message(STATUS "cleng target bundle: skipping recursive alias ${_source_path} -> ${_resolved_source}")
      return()
    endif()

    file(MAKE_DIRECTORY "${_dest_path}")
    set(_next_resolved_ancestors "${resolved_ancestors}")
    list(APPEND _next_resolved_ancestors "${_resolved_source}")
    file(GLOB _children RELATIVE "${_resolved_source}" "${_resolved_source}/*")
    foreach(_child IN LISTS _children)
      ...
      cleng_copy_resolved_tree("${source_root}" "${_child_rel}" "${dest_root}" "${_next_resolved_ancestors}")
    endforeach()
    return()
  endif()
